        ...
        this.formInfo.forEach((item, index) => {
          ...
          if (item.undealSum === '0') {
            ...
            this.markers.push({
              ...
              <span v-if='${item.state === null}' style='display: ${item.undealSum ? 'inline-block' : 'none'};padding:0 5px;height: 15px;font-size: 12px;line-height: 15px;text-align: center;color: #fff;position: absolute;border-radius: 5px 5px 5px 0;background-color: red;top: -10px;right: ${item.undealSum.length === 1 ? '-5px' : item.undealSum.length === 2 ? '-10px' : item.undealSum.length === 3 ? '-15px' : '-20px'};'>${item.undealSum}</span>
              </div>`
            })
          } else if (item.online === 1) {
            this.markers.push({
