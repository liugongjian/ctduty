        ...
        >
          <div style="width:500px;height:350px; padding: 10px 10px 0px">
            <div :model="dataDia" label-position="right" label-width="100px">
              <div prop="image" style="width:480px;height:270px;position:relative;cursor:zoom-in;" @click="()=>{openBig(dataDia.image)}">
                <!-- <el-image :src="dataDia.imageCompress" width="480" height="270" style="z-index:1;" /> -->
                <el-image :src="dataDia.imageCompress" style="width:480px;height:270px;z-index:1;" />
        ...
        this.formInfo.forEach((item, index) => {
          ...
          if (item.undealSum === '0') {
            ...
            this.markers.push({
              ...
              <span v-if='${item.state === null}' style='display: ${item.undealSum ? 'inline-block' : 'none'};padding:0 5px;height: 15px;font-size: 12px;line-height: 15px;text-align: center;color: #fff;position: absolute;border-radius: 4px 5px 5px 0;background-color: red;top: -10px;right: ${item.undealSum.length === 1 ? '-5px' : item.undealSum.length === 2 ? '-10px' : item.undealSum.length === 3 ? '-18px' : '-23px'};'>${item.undealSum}</span>
            ...
            // <div style='display: ${this.nowShowCameraId === item.id ? 'block' : 'none'};padding:0 5px;width:480px;height: 270px;font-size: 12px;line-height: 15px;text-align: center;color: #000;position: absolute;border-radius: 4px 5px 5px 0;background-color: #fff;top: -260px;right: -460px;'>${item.undealSum}</div>
          }
        })
      })
    },
