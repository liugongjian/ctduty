      ...
      if (v) {
        [].forEach.call(document.getElementsByClassName('markerImg'), function(item, index) {
          if (index === 0) {
            ...
            console.log(item.style.zIndex, 'zindex')
        ...
        this.formInfo.forEach((item, index) => {
          ...
          if (item.undealSum === '0') {
            this.markers.push({
              ...
              extData: item,
              content: `
              <div style='position: relative;z-index:${this.nowShowCameraId === item.id ? '99999999999999999999999999999999999999' : '0'}';>
                 <svg class="icon markerImg ${item.online === 1 ? 'offline' : ''}"}  id=${item.id}  slot="reference"
                  ...
                  <div class='newdialog' style='display: ${this.nowShowCameraId === item.id ? 'block' : 'none'};z-index:999999999999999999999999999999999999999999;padding:0 5px;width:480px;height: 270px;font-size: 12px;line-height: 15px;text-align: center;color: #000;position: absolute;border-radius: 5px 5px 5px 0;background-color: #fff;top: -260px;right: -460px;'>${item.undealSum}</div>
              </div>
                `
            })
            ...
            this.markers.push({
              ...
              <div class='newdialog' style='display: ${this.nowShowCameraId === item.id ? 'block' : 'none'};z-index:999999999999999999999999999999999999999999999;padding:0 5px;width:480px;height: 270px;font-size: 12px;line-height: 15px;text-align: center;color: #000;position: absolute;border-radius: 5px 5px 5px 0;background-color: #fff;top: -260px;right: -460px;'>${item.undealSum}</div>
              </div>`
            })
          } else if (item.online === 1) {
            this.markers.push({
