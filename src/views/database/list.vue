    ...
    <div v-if="isShowList" class="app-container" style="padding: 20px">
      <div class="filter-container clearfix">
        ...
        <div class="pull-right">
          <el-input v-model="formInline.searchkey" placeholder="用户名" class="filter-item" style="width: 200px;margin-left:10px;" @keyup.enter.native="onSearch"></el-input>
      ...
      <el-table ref="filterTable" :data="tableData" :header-cell-class-name="tableRowClassHeader" style="width: 100%" tooltip-effect="dark" fit class="filterNewStatusTable" @filter-change="filerStatus">
        <el-table-column :show-overflow-tooltip="true" :label="$t('database.userId')" prop="userId" min-width="10%">
          <template slot-scope="scope">
            <span>{{ scope.row.userId }}</span>
          </template>
        </el-table-column>
        <el-table-column :show-overflow-tooltip="true" :label="$t('database.userName')" prop="userName" min-width="10%">
          <template slot-scope="scope">
            <span>{{ scope.row.userName }}</span>
          </template>
        </el-table-column>
        <el-table-column :show-overflow-tooltip="true" :label="$t('database.record')" prop="record" min-width="10%">
          <template slot-scope="scope">
            <span>{{ scope.row.record }}</span>
          </template>
        </el-table-column>
        <el-table-column :show-overflow-tooltip="true" :label="$t('database.createTime')" prop="createTime" min-width="10%">
          <template slot-scope="scope">
            <span>{{ scope.row.createTime }}</span>
          </template>
        </el-table-column>
        <el-table-column :show-overflow-tooltip="true" :label="$t('database.updateTime')" prop="updateTime" min-width="10%">
          <template slot-scope="scope">
            <span>{{ scope.row.updateTime }}</span>
          </template>
        </el-table-column>
        <!-- <el-table-column :label="$t('database.operate')" width="150px">
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="detail(scope.row)">{{ '功能' }}</el-button>
          </template>
        </el-table-column> -->
      </el-table>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      fetchAllOperateList(params).then(response => {
